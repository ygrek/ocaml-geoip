...
(* OASIS_START *)
(* DO NOT EDIT (digest: 9fdc9f61282081513e96d481e8666fe5) *)
(*
...
# 21 "src/oasis/OASISLibrary.ml"
  ...
  let generated_unix_files ~ctxt (cs, bs, lib) 
    ...
    (* Find the possible base paths for module *)
    let choose_base modul =
           let base_paths = List.map (OASISUnixPath.concat bs.bs_path)
              [modul; String.uncapitalize modul; String.capitalize modul]
           in
           try
             let base_fn = 
               List.find
           ...
           with Not_found ->
             OASISMessage.warning
               ...
               modul cs.cs_name;
               (* return all possibilities, BaseBuilt will figure out *)
               base_paths
    in
    (* The objects that should be compiled along *)
    let objects = ref [] in
    let tack_objects ext modules = 
      List.iter (fun modul ->
        objects := (List.map (fun name -> name^ext) (choose_base modul)) :: !objects)
      modules
    in
    tack_objects ".cmi" lib.lib_modules;
    if is_native () then
    begin
      tack_objects ".cmx" lib.lib_modules;
      tack_objects ".cmx" lib.lib_internal_modules;
    end;
      ...
      in
        ...
        try
          Arg.parse_argv
            ...
            (Arg.align args)
            (fail (f_ "Don't know what to do with arguments: '%s'"))
  ...
  let all t args =
    ...
    in
      Arg.parse_argv
        ...
        (fail (f_ "Don't know what to do with '%s'"))
        "";
    ...
    List.iter 
      (fun (ev, data) ->
         if ev = install_file_ev then
           ...
           fail (f_ "Unknown log event '%s'") ev;
     ...
     package =
       {
          ...
          maintainers = [];
          authors = ["ygrek"];
          homepage = Some "http://geoip.forge.ocamlcore.org/";
               ...
               Library
                 ({
                     ...
                     },
                   {
                      ...
                      bs_c_sources = ["geoip_stubs.c"; "geoip_init.h"];
                      bs_data_files = [];
               ...
               Doc
                 ({
                     ...
                     },
                   {
                      ...
                      doc_install_dir = "$datarootdir/doc/ocaml-geoip";
               ...
               SrcRepo
                 ({
                     ...
                     },
                   {
                      src_repo_type = Git;
                      src_repo_location =
                        "http://git.ocamlcore.org/geoip/geoip.git";
                      src_repo_browser =
                        Some
                          "http://git.ocamlcore.org/cgi-bin/gitweb.cgi?p=geoip/geoip.git";
          ...
          plugins = [(`Extra, "DevFiles", None); (`Extra, "META", None)];
